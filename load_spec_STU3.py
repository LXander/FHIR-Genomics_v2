...
# Adding the logging system at load_spec.log
logging.basicConfig(level=logging.DEBUG,
                format='%(asctime)s %(filename)s[line:%(lineno)d]: %(levelname)s %(message)s \n',
                datefmt='%a, %d %b %Y %H:%M:%S',
                filename='load_spec.log',
                filemode='w')


#TODO: Need improvement
'''
    I'm not sure if there is anychange that correlated to
    the inner code implementation.
'''
...
def find_extension_attri(filename):
    try:
        ...
        logging.info('Cannot find %s.json, trying a case-lower one' % filename)
        #pass
    try:
        filepath = os.path.join(spec_dir, 'extension-' + filename.lower() + '.json')
        #print filepath
        with io.open(filepath, encoding='utf-8') as handle:
            parsed = json.load(handle)
            ori_elements = parsed['differential']['element']
        ...
        logging.info('Cannot find %s.json' % filename.lower())
        logging.warning('Cannot find %s.json and %s.json' %(filename,filename.lower()));
...
def process_profile(profile):
    ...
    for ori_element in ori_elements:
        ...
        if '.extension' in ori_element['path'] and ori_element.get('name') is not None:
            ...
            # Here is the change between 2016jan and 2016 Sep
            # In 2016 Jan it should be ['profile'][0]
            extension_full_name = ori_element['type'][0]['profile'].split('/')[-1]
                ...
                if extension_type:
                    ...
                    if extension_type_code == 'Reference':
                        ...
                        for i in extension_type:
                            ...
                            reference_type = i['profile'].split('/')[-1]
            ...
            for each_type in element_type:
                ...
                #This place here only deals with triple layer situation with a '-' connection
                names = path.split('.')[1:]
                name = '-'.join(names)
...
def load_spec(spec_dir):
    ...
    with open('fhir/fhir_spec_test.py', 'w') as spec_target:
        spec_target.write("'''\n%s\n'''" % WARNING)
        spec_target.write('\n')
